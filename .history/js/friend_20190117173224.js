...
$(function () {
    ...
    (function(){
        if(friendRecordInfo.length === 0){
            $('div.record-box>p').show();
            $('#record-list').hide();
        }else{
            $('div.record-box>p').hide();
            $('#record-list').show();
        }
        var str = '';
        var scoreIndex = lanternsArr[friendRecordInfo[i].myLanType][friendRecordInfo[i].friendLanType-1].percent;
        for(var i = 0; i < friendRecordInfo.length; i++){
            str += '<li><img src="'+friendRecordInfo[i].friendPhoto+'"><p><span>'+friendRecordInfo[i].friendName+'</span><span class="_percent">'+lanternsArr[friendRecordInfo[i].myLanType][friendRecordInfo[i].friendLanType-1].percent+'</span></p><p class="_score">'+friendRecordInfo[scoreIndex].score+'</p><a href="javascript:void(0)"></a><i id="mlt" class="'+friendRecordInfo[i].myLanType+'"></i><i id="flt" class="'+friendRecordInfo[i].friendLanType+'"></i></li>';
        }
