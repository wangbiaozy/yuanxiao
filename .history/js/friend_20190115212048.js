...
$(function () {
    ...
    var myLanType = Math.floor(Math.random() * 9) + 1;
    var imgPath = './images/';
    ...
    console.log(myLanType,friendLanType,lanternsArr[myLanType][friendLanType-1]);
    

    //好友入口弹窗
    (function(){
        $('.hd-name').text(lanternsName[friendLanType-1]);
        $('.friend').attr("src",imgPath + "r_" + friendLanType + ".png");//此处路径取好友花灯图片路径
        $('.my-hd').attr("src",imgPath + "l_" + myLanType + ".png");
        $('.percent').text(lanternsArr[myLanType][friendLanType].percent);
        $('#tofriend').show();
        showMask();
    })();
    console.log(lanternsArr[myLanType][friendLanType].percent)
    $('.percent').text(lanternsArr[myLanType].percent);
    //入口弹窗拼花灯按钮
